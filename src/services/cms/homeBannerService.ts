...
import { IUpdateHomeBannerBody } from "../../types/requests";
...
export class HomeBannerService {
    ...
    async createHomeBanner(
        ...
        if (existingHomeBanner) {
            throw new ApiError(RESPONSE_CODE.BAD_REQUEST, "A 'HomeBanner' entry already exists. You cannot create another one.");
        }


        // Process uploaded images
        const homeBannerPhotos = files.map((file: any) => ({
            fileName: file.filename,
        }));

        // Prepare the gallery data
        const newGallery = new HomeBanner({
            CreatedBy: userId,
    ...
    async updateHomeBanner(
        payload: Partial<IUpdateHomeBannerBody>,
        homeBannerId: string,
        files: any // Array of image files
    ): Promise<any> {
        // Validate that we have a gallery id to update.
        if (!homeBannerId) {
            throw new ApiError(RESPONSE_CODE.NOT_FOUND, "Home BannerId ID is required", {}, false);
        }

        // Find the gallery document by ID.
        const gallery = await HomeBanner.findById(homeBannerId);

        if (!gallery) {
            throw new ApiError(RESPONSE_CODE.NOT_FOUND, "Home Banner Not Found", {}, false);
        }
        ...
        if (deleteIds.length > 0) {
            ...
            if (deleteFileNames.length > 0) {
                const deletePromises = deleteFileNames.map((filename) => {
                    const filePath = path.join("uploads", "homeBanner", CreatedById, filename);
    ...
    async getHomeBanner(): Promise<any> {
        ...
        const gallery = await HomeBanner.findOne().lean();
        if (!gallery) {
        ...
        gallery.Photos = gallery.Photos.map(photo => ({
            ...photo,
            imageUrl: this.fileService.getFilePathFromDatabase(FOLDER_NAMES.HOMEBANNER, [createdByIdString, photo.fileName])
        }));
